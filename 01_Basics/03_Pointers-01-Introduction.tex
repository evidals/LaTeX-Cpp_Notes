...
%~\cref{subsec:nullptr}
Pointers should be initialized to a \CppCommonCode{nullptr} or to a memory address.\\
\begin{minipage}{\MPWxXSxLISTING\textwidth} % = 0.36 times \textwidth
\vspaceTextToListing % =\vspace{0.1cm}
\begin{CPPCode}
...
\end{minipage}
\begin{itemize}
    \item Now the variable \CppVar{yPtr} is said to point to \CppVar{y}.
    \item Also \CppVar{yPtr} indirectly references variable \CppVar{y}'s value (i.e. 5).
    \item When using \textquotesingle\CppCommonCode{\&}\textquotesingle{} in expressions like \CppVar{\&y}, it is called \textbf{address operator}.
\end{itemize}

\subsection{The indirection operator \CppCommonCode{(*)}}
